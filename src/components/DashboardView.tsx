...
import { 
  ...
  Sparkles, Award, Bot, Download, Network
} from 'lucide-react';
...
import { AreaChart, Area, LineChart, Line, Legend, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ReferenceArea } from 'recharts';
...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-950/90 border border-blue-500/10 p-3 rounded-xl shadow-xl backdrop-blur-sm">
        <p className="text-[10px] font-mono text-slate-300 mb-2 border-b border-blue-500/10 pb-1">Дата: {label}</p>
...
export default function DashboardView({ onSelectTab, onSelectEntity }: DashboardViewProps) {
    ...
    const unsubscribe = onSnapshot(dashboardDoc, (snapshot) => {
      if (snapshot.exists()) {
        ...
        setDoc(dashboardDoc, {
          ...
          handleFirestoreError(err, OperationType.WRITE, 'dashboard/risk_metrics');
        });
      }
    }, (error) => {
      console.error("Firestore Error: ", error);
      handleFirestoreError(error, OperationType.LIST, 'dashboard/risk_metrics');
    });
  ...
  const stats = [
    { label: "Інтегровані Джерела", value: "84 API", change: "+3 автоматично за 24г", icon: Network, color: "text-blue-400 bg-blue-500/10 border-blue-500/30" },
    { label: "Автоматичні Тести", value: "99.9%", change: "Contract & Chaos Tests", icon: ShieldAlert, color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" },
    { label: "Вузлів у Графі Знань", value: "148.2M", change: "+1.2M за годину", icon: Sparkles, color: "text-amber-400 bg-amber-500/10 border-amber-500/20" },
    { label: "Згенеровано Коду", value: "4.2M LOC", change: "100% без людини", icon: Terminal, color: "text-purple-400 bg-purple-500/10 border-purple-500/20" }
  ];
  ...
  return (
    <div className="space-y-6" id="dashboard-view-root">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" id="hud-stats-grid">
        {stats.map((stat, i) => (
          <motion.div 
            ...
            className="bg-slate-900/40 border border-blue-500/10 hover:border-blue-500/50 rounded-2xl shadow-[0_4px_30px_rgba(0,0,0,0.5)] backdrop-blur-md p-4.5 flex items-center justify-between shadow-xl backdrop-blur-sm relative overflow-hidden group cursor-pointer"
          >
            ...
            <div className="relative z-10">
              ...
              <span className="text-[10px] text-blue-400 font-mono font-bold block mt-1">
                {stat.change}
              </span>
            </div>
      ...
      <div className="bg-gradient-to-r from-blue-900/20 to-slate-900/40 border border-blue-500/20 rounded-2xl shadow-xl p-5 relative overflow-hidden backdrop-blur-md">
        <div className="absolute top-0 right-0 p-4 opacity-10 pointer-events-none">
          <Bot className="w-24 h-24 text-blue-500" />
        </div>
        <div className="flex items-start gap-4 relative z-10">
          <div className="p-3 bg-blue-500/10 border border-blue-500/30 rounded-xl shrink-0">
            <Sparkles className="w-5 h-5 text-blue-400 animate-pulse" />
          </div>
          <div className="space-y-2">
            <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
              Autonomous Factory Summary
              <span className="bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded text-[9px] font-mono border border-emerald-500/30">AGENT MODE</span>
            </h3>
            <p className="text-sm text-slate-300 leading-relaxed font-medium max-w-4xl">
              За останні 24 години <strong className="text-emerald-400 font-bold">Discovery Engine виявив 3 нових API.</strong> 
              Codegen Engine автоматично згенерував ETL-клієнти, створив SQL DDL міграції та Qdrant Embedding Loaders без помилок статичного аналізу. 
              Система <strong className="text-blue-400">успішно пройшла GitOps пайплайн</strong> та задеплоєна в Production.
            </p>
            <div className="flex gap-3 pt-2">
              <button className="text-[10px] font-bold text-blue-400 hover:text-blue-300 uppercase tracking-wider flex items-center gap-1 transition-colors cursor-pointer">
                Генерувати ADR звіт <span className="text-lg leading-none">&rarr;</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-8 space-y-6">
          ...
          <div className="bg-slate-900/40 border border-blue-500/5 rounded-2xl p-5 shadow-lg space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
              <div className="md:col-span-8 h-[240px] bg-slate-950/40 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.5)] border border-slate-850 rounded-xl relative overflow-hidden flex items-center justify-center">
                ...
                <GeospatialHeatmap points={filteredHeatmapPoints} width={800} height={400} showD3Hotspots={showD3Hotspots} />
                ...
                <div className="absolute top-3 left-3 flex flex-wrap gap-2 z-10">
                  ...
                  <button
                    ...
                    className={`px-2 py-1 rounded text-[8px] font-mono font-bold border transition-all cursor-pointer flex items-center gap-1 ${
                      showD3Hotspots 
                  ...
                  >
                    <span className={`w-1 h-1 rounded-full ${showD3Hotspots ? 'bg-rose-500 animate-ping' : 'bg-slate-600'}`}></span>
              ...
              <div className="md:col-span-4 bg-slate-950/40 border border-blue-500/5 rounded-xl p-3.5 flex flex-col justify-between">
          ...
          <div className="bg-slate-900/40 border border-blue-500/10 rounded-2xl shadow-[0_4px_30px_rgba(0,0,0,0.5)] backdrop-blur-md p-5 shadow-xl space-y-4 relative overflow-hidden backdrop-blur-sm" id="risk-distribution-heatmap-widget">
              ...
              <div className="flex items-center gap-1.5 bg-slate-950/80 p-1 rounded-xl border border-blue-500/5/60">
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all cursor-pointer ${
                    heatmapFilter === 'all' ? 'bg-blue-600 text-white shadow' : 'text-slate-300 hover:text-slate-200'
                  }`}
                >
                  Всі
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all cursor-pointer ${
                    heatmapFilter === 'company' ? 'bg-blue-600 text-white shadow' : 'text-slate-300 hover:text-slate-200'
                  }`}
                >
                  Юрособи
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all cursor-pointer ${
                    heatmapFilter === 'person' ? 'bg-blue-600 text-white shadow' : 'text-slate-300 hover:text-slate-200'
                  }`}
                >
                  Особи
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all cursor-pointer ${
                    heatmapFilter === 'cryptowallet' ? 'bg-blue-600 text-white shadow' : 'text-slate-300 hover:text-slate-200'
                  }`}
                >
                  Крипто
                </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 relative z-10">
              {/* Heatmap main canvas */}
              <div className="lg:col-span-8 relative h-[300px] bg-slate-950/80 border border-blue-500/5/80 rounded-xl overflow-hidden p-4 flex flex-col justify-between">
                  ...
                  {filteredEntities.map((ent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`w-6 h-6 rounded-full border-2 border-slate-950 bg-slate-900/50 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.3)] flex items-center justify-center shadow-lg shadow-black/80 group-hover:scale-125 group-hover:border-blue-400 transition-all duration-300 relative`}>
                          <Icon className={`w-3.5 h-3.5 ${riskText}`} />
                          
                          {/* Risk Score Pill directly attached */}
                          <span className="absolute -top-3.5 -right-3 px-1 rounded bg-slate-950/40 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.5)] border border-blue-500/10 text-[7px] font-mono font-bold text-slate-300 scale-90 group-hover:scale-100 transition-transform">
                            {ent.riskScore}%
                          </span>
                        </div>

                        {/* Floating quick mini-label */}
                        <span className="absolute left-7 top-1/2 -translate-y-1/2 whitespace-nowrap bg-slate-950/90 border border-blue-500/5/60 px-2 py-0.5 rounded text-[8.5px] font-mono font-bold text-slate-300 group-hover:text-white transition-colors">
                          {ent.name.replace(/ТОВ |"|'/g, '')}
                        </span>

                        {/* Custom hover detail panel */}
                        {isSelected && (
                          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-[240px] bg-slate-950/95 border border-blue-500/30 p-3 rounded-xl shadow-2xl z-30 pointer-events-none space-y-1 text-left backdrop-blur-md">
                            ...
                            <div className="border-t border-blue-500/5/60 pt-1 mt-1 flex items-center justify-between">
                              <span className="text-[7.5px] text-blue-400 font-mono animate-pulse">Клікніть для повного аналізу зв'язків</span>
                ...
                {/* Chart X-Axis Labels */}
                <div className="flex justify-between text-[8px] font-mono text-slate-500 border-t border-blue-500/5 pt-2 ml-14 mr-4 select-none">
              ...
              <div className="lg:col-span-4 bg-slate-950/40 border border-blue-500/5 rounded-xl p-4 flex flex-col justify-between space-y-4">
                <div className="space-y-3.5">
                  <span className="text-[9px] text-slate-500 font-mono font-bold uppercase tracking-widest block border-b border-blue-500/5 pb-1.5">
                    Показники ризик-матриці
                  </span>
                  
                  {/* Analytics metrics */}
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-slate-950/80 p-2.5 rounded-lg border border-blue-500/5 text-center">
                      <span className="text-[8px] text-slate-500 font-mono block">СЕРЕДНІЙ РИЗИК</span>
                      <span className="text-lg font-black text-blue-400 font-mono">{avgRiskScore}%</span>
                    </div>
                    <div className="bg-slate-950/80 p-2.5 rounded-lg border border-blue-500/5 text-center">
                  ...
                  {/* Settings toggle */}
                  <div className="bg-slate-950/80 rounded-lg border border-blue-500/5 p-3 space-y-2">
                  ...
                  <div className="space-y-1.5 pt-1">
                    ...
                    <div className="flex flex-col gap-1 text-[9px] text-slate-300 bg-slate-950/40 p-2 rounded border border-blue-500/5/60 space-y-1">
                      <div className="flex items-center justify-between">
                ...
                <div className="text-[9px] text-slate-500 font-mono leading-relaxed border-t border-blue-500/5/80 pt-3">
          ...
          <div className="bg-[#0b1329]/40 border border-slate-850 rounded-2xl p-5 shadow-xl space-y-4 relative overflow-hidden backdrop-blur-sm" id="tactical-interactive-panel">
            ...
            <div className="flex items-center justify-between border-b border-blue-500/5 pb-3 relative z-10">
              <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
              ...
              <div className="bg-slate-950/70 border border-blue-500/5 rounded-xl p-3.5 flex flex-col justify-between space-y-4 hover:border-blue-500/30 transition-all group">
                <div>
                  <h4 className="text-xs font-bold text-slate-200 group-hover:text-blue-400 transition-colors">📡 ГЛОБАЛЬНИЙ РАДАР</h4>
                ...
                <div>
                  {radarStatus === 'SCANNING' && (
                    <div className="flex items-center gap-2 text-[10px] text-blue-400 font-mono mb-2 font-bold">
                  ...
                  <button
                    ...
                    className={`w-full py-2 px-3 text-[10px] font-black uppercase rounded-lg tracking-wider font-mono cursor-pointer transition-all ${radarStatus === 'SCANNING' ? 'bg-slate-900/50 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.3)] text-slate-500' : 'bg-blue-600 hover:bg-blue-500 text-white shadow-md shadow-blue-500/10'}`}
                  >
              ...
              <div className="bg-slate-950/70 border border-blue-500/5 rounded-xl p-3.5 flex flex-col justify-between space-y-4 hover:border-blue-500/30 transition-all group">
                <div>
                  <h4 className="text-xs font-bold text-slate-200 group-hover:text-blue-400 transition-colors">🔄 СИНХРОНІЗАЦІЯ БАЗИ</h4>
                ...
                <div>
                  ...
                  <button
                    ...
                    className={`w-full py-2 px-3 text-[10px] font-black uppercase rounded-lg tracking-wider font-mono cursor-pointer transition-all ${syncStatus === 'SYNCING' ? 'bg-slate-900/50 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.3)] text-slate-500' : 'bg-blue-600 hover:bg-blue-500 text-white shadow-md shadow-blue-500/10'}`}
                  >
              ...
              <div className="bg-slate-950/70 border border-blue-500/5 rounded-xl p-3.5 flex flex-col justify-between space-y-4 hover:border-blue-500/30 transition-all group">
                <div>
                  <h4 className="text-xs font-bold text-slate-200 group-hover:text-blue-400 transition-colors">🧪 ШІ AML СКРИНІНГ</h4>
                ...
                <div>
                  ...
                  <button
                    ...
                    className="w-full py-2 px-3 text-[10px] font-black uppercase rounded-lg tracking-wider font-mono bg-blue-600 hover:bg-blue-500 text-white cursor-pointer transition-all shadow-md shadow-blue-500/10"
                  >
                    Запустити AML ШІ
                  </button>
                </div>
          ...
          {/* Risk Dynamics Chart */}
          <div className="bg-slate-900/40 border border-blue-500/5 rounded-2xl p-5 shadow-lg relative overflow-hidden" id="risk-dynamics-chart">
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[10px] font-mono font-bold uppercase tracking-wider border transition-colors ${showForecast ? 'bg-sky-500/20 text-sky-400 border-sky-500/30' : 'bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-sky-400 border-slate-700'}`}
                >
                ...
                {zoomIndices && (
                  <button 
                    onClick={zoomOut}
                    className="flex items-center gap-1.5 px-2.5 py-1.5 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 rounded-lg text-[10px] font-mono font-bold uppercase tracking-wider border border-blue-500/30 transition-colors"
                  >
                    <span>Zoom Out</span>
                  </button>
                )}
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-blue-400 rounded-lg text-[10px] font-mono font-bold uppercase tracking-wider border border-slate-700 transition-colors"
                title="Download CSV"
              >
          ...
          <div className="bg-indigo-950/10 border border-indigo-900/30 rounded-2xl p-5 relative overflow-hidden" id="ai-insights-block">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold uppercase text-blue-400 tracking-widest font-mono">AI SUMMARY & РЕКОМЕНДАЦІЇ ШІ</span>
            </div>
        ...
        <div className="xl:col-span-4 space-y-6">
          ...
          <div className="bg-slate-900/40 border border-blue-500/5 rounded-2xl p-5 shadow-lg space-y-3.5">
            ...
            <div className="space-y-2.5">
              {recentSearches.map((search, i) => (
                <div 
                  ...
                  className="bg-slate-950/70 border border-blue-500/5 rounded-xl p-3 flex items-center justify-between transition-all duration-300 ease-out cursor-pointer group hover:bg-slate-900/60 hover:border-blue-400/40 hover:-translate-y-[1px] hover:shadow-[0_4px_15px_rgba(99,102,241,0.1)]"
                >
                  <div className="flex items-center gap-2.5">
                    <div className="p-1.5 rounded-lg bg-slate-900/50 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.3)] border border-blue-500/10 text-slate-300 group-hover:text-blue-400 transition-colors">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-200 group-hover:text-blue-400 transition-colors">{search.text}</p>
          ...
          <div className="bg-slate-900/40 border border-blue-500/5 rounded-2xl p-5 shadow-lg space-y-3.5">
            ...
            <div className="space-y-3">
              {criticalRisks.map((risk, idx) => (
                <div key={idx} className="bg-slate-950/40 backdrop-blur-md shadow-[0_4px_30px_rgba(0,0,0,0.5)] p-3 rounded-xl border border-blue-500/5 space-y-1.5">
